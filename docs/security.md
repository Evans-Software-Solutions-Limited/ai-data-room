...
| **NFR2**  | Passwords hashed via KDF, never stored/logged/transmitted by us           | Delegated entirely to WorkOS AuthKit. Our codebase never declares a `password` field on any type.                                                                                                                                                                                                                                                                  | Matrix NFR2 grep across `microservices/**` + `packages/**` for `password: (z.\|string)`.                                                                                                         |
