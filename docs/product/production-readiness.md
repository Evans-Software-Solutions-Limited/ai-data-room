...
| RB-7 | **Role vocabulary migration** — shipped DB enum is `owner/admin/internal`; design + specs now use `owner/editor/viewer`.                                                                                              | auth-and-orgs               | Medium                    | Rename `admin`→`editor`, `internal`→`viewer` (enums, repos, guards, `/me`, WorkOS metadata, tests). See [ADR-012](../../adr/012-role-vocabulary.md). Land before access-control (slice 3). |
