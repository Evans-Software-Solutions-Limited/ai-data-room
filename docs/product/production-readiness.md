...
- **Slices 2–17:** specs complete (requirements + design + tasks drafted), **no
  ...
  — is unbuilt.
- **Net:** we have a production-grade _auth foundation_, not a product. The
...
| RB-7 | **Role vocabulary migration** — shipped DB enum is `owner/admin/internal`; design + specs now use `owner/editor/viewer`.                                                                                              | org-provisioning (T-000)    | Medium                    | Rename `admin`→`editor`, `internal`→`viewer` (enums, repos, guards, `/me`, WorkOS metadata, tests). See [ADR-012](../../adr/012-role-vocabulary.md). **Folded into org-provisioning T-000** — the first build target — so it lands before any role-tier logic. |
...
question → get a cited answer" loop — sequence the slices for _demo value_, not
