// Integration tests for `AuditRepo` (writer, unscoped) and
// `ScopedAuditReadRepo` (T-004 scoped read).
//
// Tenant-isolation (slice 10) / T-004 split the org-scoped read off
// the writer: `AuditRepo.write` stays unscoped (nullable `org_id` for
// system / no-local-actor events), while the org-scoped listing moved
// to `ScopedAuditReadRepo.list()`.
...
import {
  AuditRepo,
  ScopedAuditReadRepo,
} from "../../../src/infrastructure/db/auditRepo";
...
describe("ScopedAuditReadRepo (integration) — T-004 scoped read", () => {
  let db: PostgresJsDatabase<typeof schema>;
  let audit: AuditRepo;
  let users: UserRepo;
  let orgs: OrgRepo;
  ...
  it("list() returns events newest-first within the bound org", async () => {
    const { org, actor } = await seedOrgAndActor("listbyorg");
    const auditReads = new ScopedAuditReadRepo(db, org.id);
    ...
    const list = await auditReads.list();
  ...
  it("list() honours the limit and the keyset cursor", async () => {
    ...
    const page1 = await auditReads.list({ limit: 2 });
    expect(page1).toHaveLength(2);
    const cursor = page1[page1.length - 1];

    const page2 = await auditReads.list({
      limit: 2,
  ...
  it("list() pagination respects the (occurredAt, id) tiebreaker", async () => {
    ...
    const all = await auditReads.list();
    ...
    for (let page = 0; page < 3; page++) {
      const rows = await auditReads.list({ limit: 2, before: cursor });
  ...
  it("list() does not return events from other orgs, or NULL-org system rows", async () => {
    ...
    // A NULL-org system row (e.g. pre-org signup) must not leak into
    // ANY org's scoped view — this is the registry's documented
    // behaviour for `audit_events.org_id` being nullable.
    await audit.write({
      eventType: "signup",
      outcome: "success",
      orgId: null,
    ...
    const listB = await new ScopedAuditReadRepo(db, orgB.id).list();
    expect(listB).toHaveLength(0);

    const listA = await new ScopedAuditReadRepo(db, orgA.id).list();
    expect(listA).toHaveLength(1);
    expect(listA[0]?.eventType).toBe("login_success");
  });
});
